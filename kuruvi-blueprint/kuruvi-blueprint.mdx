...
date: "2026-02-15"
description: "How I turned a simple WhatsApp bot into a self-organizing team of AI agents — each with its own identity, its own Discord presence, and its own specialty. No frameworks. No LangChain. Just files."
...
Seven AI agents. Each with a name, a personality, a Discord identity.

| Agent | Specialty | What They Do |
|-------|-----------|--------------|
| **Ada** | Architect | Designs systems, evaluates approaches, catches edge cases before code is written |
| **Turing** | Developer | Writes code, refactors, fixes bugs — speaks fluent TypeScript/Python |
| **Nitty** | QA Engineer | Tests everything, breaks things on purpose, validates fixes with screenshots |
| **Steve** | Product | Gathers requirements, challenges assumptions, keeps the roadmap honest |
| **Jony** | Designer | UI reviews, accessibility checks, visual feedback with an eye for detail |
| **Wanderer** | Researcher | Searches the web, finds docs, digests RFCs and blog posts |
| **Helix** | Coordinator | Delegates work, monitors completions, keeps humans in the loop |
...
- **Container isolation** — every agent invocation runs in a fresh Docker container
- **Persistent memory** — `CLAUDE.md` files that survive across sessions
- **Scheduled tasks** — cron, interval, or one-shot
- **File-based IPC** — containers talk to the host by writing JSON files to a shared directory
...
**A personality file.** `groups/turing/CLAUDE.md` opens with:

```markdown
# Turing — Developer Agent

You are Turing, inspired by Alan Turing — the father of computer science.

## Your Role

You are a senior developer who ships clean, maintainable code. You:

- Write TypeScript and Python with strong typing
- Prefer simple solutions over clever ones
- Add tests. Always. Unless Ada explicitly says not to.
- Leave code better than you found it

## When Someone Asks You To Build Something

1. Understand the problem first — ask clarifying questions if needed
2. Sketch the approach, then write code
3. Run tests. Fix failures. Run again.
4. Push to main and describe what changed

## What You Don't Do

- Don't design systems — ask Ada
- Don't write copy — ask Steve
- Don't touch CSS unless it breaks accessibility — ask Jony
```

These aren't cosmetic. Ada approaches problems differently than Turing. She thinks in systems. He thinks in code. The CLAUDE.md file is the entire personality specification.
...
That's the entire task management system. A real task looks like this:

```json
{
  "title": "Add OAuth login with Google",
  "description": "Implement Google OAuth using NextAuth.js. User should be able to sign in with their Google account. Store the Google ID in the users table.",
  "agent": "turing",
  "status": "queued",
  "priority": "high",
  "artifacts": [
...
Tasks flow through four states: `queued → in_progress → completed` (or `blocked`).
...
One task per agent per cycle. Fresh session per task — no context bleeds between independent work.
...
4. Don't wait for Helix queue — agents report to their own queues
```
...
- **0 frameworks** — no LangChain, no CrewAI, no LangGraph
- **60 seconds** — task poller interval
- **30 minutes** — max container timeout per task
...
They're solving problems you *may* never have.

The real insight isn't that you need a framework to coordinate agents. It's that you *don't*. Claude follows instructions written in plain English. Docker is good enough to isolate agents. A JSON file in a directory is good enough for IPC. A 3-table database is good enough for a task queue.

The benefits I found:

- **Debugging at 2am** is easy. No logs to parse. Just read the JSON files.
- **Changing behavior** is one file edit away. No framework to learn.
- **Adding an agent** takes 30 minutes, not a sprint planning session.
- **Owning the stack** means every bug is yours to fix. No vendor blaming.
...
*Kuruvi is built on [NanoClaw](https://github.com/gavrielc/nanoclaw), an open-source personal Claude assistant. The name means "small bird" in Tamil — because the best systems are light enough to fly.*
